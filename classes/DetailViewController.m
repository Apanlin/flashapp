...
- (void)viewDidLoad
{
    ...
    self.detailTableView.backgroundColor = RGB(48, 48, 50);
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if (![appDetail.oprice isEqualToString:@"0.00000"]) {
        [anzhuangBtn setTitle:[NSString stringWithFormat:@"￥%.2f",[appDetail.oprice floatValue]] forState:UIControlStateNormal];
    }else{
        [anzhuangBtn setTitle:[NSString stringWithFormat:@"免费"] forState:UIControlStateNormal];
    }
    ...
    //网络请求数据,然后赋值
    [myNetWork startAppXiangQingWithApid:appDetail.apid completion:^(NSDictionary *result){
//        NSLog(@"result is %@",result);
        appDetail.apdesc =  [result objectForKey:@"apdesc"];
//        NSLog(@"%@",appDetail.apdesc);
