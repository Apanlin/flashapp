...
{
    sqlite3* conn = [DBConnection getDatabase];
    char* sql = "select id, totalBefore, totalAfter, createtime from stats_day where accessDay = ?";
    
    Statement* stmt = [[Statement alloc] initWithDB:conn sql:sql];
    [stmt bindInt64:day forIndex:1];
    ...
    if ( [stmt step] == SQLITE_ROW ) {
        ...
        stats.accessDay = day;
        stats.createTime = [stmt getInt32:3];
    }
...
{
    sqlite3* conn = [DBConnection getDatabase];
	char* sql = "SELECT  min(accessDay) lastDay  FROM STATS_DAY";//查询出第一次访问的最小时间 和最后一次时间
	time_t firstDay = 0;
	Statement* stmt = [[Statement alloc] initWithDB:conn sql:sql];	
	if ( [stmt step] == SQLITE_ROW ) {
