...
+ (void) explainAccessLogOnlyToday:(StatsDay*)statsDay
{
    ...
    //更新stats_day表
    StatsDay* lastDayStats = [StatsDayDAO getStatsDay:statsDay.accessDay];
    if ( lastDayStats ) {
...
+ (void) explainAccessLogHistory:(time_t)lastDayLong data:(NSArray*)statsArray
{
    ...
    time_t firstTimeOfDay = 0;
    if ( lastDayLong > 0 ) {
        firstDayOfMonth = [DateUtils getFirstDayOfMonth:lastDayLong];
        firstTimeOfDay = [DateUtils getFirstTimeOfDay:lastDayLong];
    }
    
    if ( firstTimeOfDay > 0 ) {
        [StatsDayDAO deleteStatsDay:firstTimeOfDay];
        [StatsDetailDAO deleteStatsDetail:firstTimeOfDay];
    }
...
+ (void)explainAccessLog:(time_t)lastDayLong serverTime:(time_t)serverTime data:(NSArray*)statsArray
{
    ...
    if ( isToday ) {
        ...
        [self explainAccessLogOnlyToday:statsDay];
    }
    else {
