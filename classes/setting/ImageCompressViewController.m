...
- (void)dealloc {
    ...
    [_changeGreenLine release];
    [_changeBlackLine release];
    [_btnBgView release];
...
- (void)viewDidUnload {
    ...
    [self setChangeBlackLine:nil];
    [self setChangeGreenLine:nil];
...
- (void)viewDidLoad
{
    ...
    [_saveBtn setTitleColor:[UIColor colorWithRed:115.0/255.0 green:115.0/255.0 blue:115.0/255.0 alpha:1.0] forState:UIControlStateDisabled];
    _saveBtn.titleLabel.font = [UIFont systemFontOfSize:12.0f];
    [_saveBtn setTitleColor:[UIColor colorWithRed:0.0 green:115.0 blue:0.0 alpha:1.0] forState:UIControlStateNormal];
    _saveBtn.frame = CGRectMake(0, 0, 45 , 35);
    [_saveBtn addTarget:self action:@selector(saveBtnPressed:) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *rightBar = [[UIBarButtonItem alloc] initWithCustomView:_saveBtn];
//    rightBar.enabled = NO;
...
{
    CGFloat selBtnX;
    CGRect rect = self.changeGreenLine.frame;
    for (int i = 1 ;  i <=4 ; i++) {
        ...
        if (btn.tag == selBtn.tag) {
            ...
            [UIView animateWithDuration:0.2 animations:^{
                self.changeGreenLine.frame = CGRectMake(selBtnX, rect.origin.y, rect.size.width, rect.size.height);
            }];
        }else{
            btn.enabled = YES;
        }
