...
+ (NSString*) composeURLVerifyCode:(NSString*)url appendStr:(NSString*)appendStr
{
    ...
    NSString* s = [NSString stringWithFormat:@"%@%@%@%d%@", deviceId, CHANNEL, API_KEY, rd,
                   appendStr?appendStr:@""];
    ...
    if ( range.location == NSNotFound ) { //判断如果有问号就不用加问号，如果没有就加问号
                     ...
                     response:(NSHTTPURLResponse**)response error:(NSError**)error
{
	...
	NSMutableURLRequest* req = [NSMutableURLRequest requestWithURL:finalURL
												   ...
												   timeoutInterval:NETWORK_TIMEOUT];    //无视任何缓存策略,无论是本地的还是远程的,总是从原地址重新下载
    ...
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
	NSData* data = [NSURLConnection sendSynchronousRequest:req returningResponse:response error:error]; //同步加载URL请求的方法
...
- (void)get:(NSString*)aURL
{
    ...
    //NSURLRequestReloadIgnoringLocalAndRemoteCacheData无视任何缓存策略,无论是本地的还是远程的,总是从原地址重新下载
	NSMutableURLRequest* req = [NSMutableURLRequest requestWithURL:finalURL
                                         cachePolicy:NSURLRequestReloadIgnoringLocalAndRemoteCacheData
                                     timeoutInterval:NETWORK_TIMEOUT];
    
    [self addAuthHeader:req];//设置请求的头文件
...
-(void)post:(NSString*)aURL data:(NSData*)data
{
	...
	NSURL *finalURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@source=%@", 
											URL,
											([URL rangeOfString:@"?"].location != NSNotFound) ? @"&" : @"?" , 
											kOAuthConsumerKey]];//kOAuthConsumerKey 是什么？
    ...
    [req setHTTPMethod:@"POST"];
    //设置Content-Type （二进制传输，防止遇到0后失败 ， 所以要加这个 为了防止出现错误吧;
