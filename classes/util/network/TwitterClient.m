...
+ (void) getMemberInfoSync
{
    ...
    NSString* url = [NSString stringWithFormat:@"%@/%@.json?username=%@&useVal=%@&startQuantity=%.1f&shareQuantity=%.1f&currTime=%lld&platform=%@&osversion=%@&deviceToken=%@&vr=%@", 
                     API_BASE, API_LOG_MEMBERINFO, 
                     username ? [username encodeAsURIComponent] : @"", 
                     tcUsed ? tcUsed : @"",
                     user.dayCapacityDelta, user.monthCapacityDelta, currTime,
                     plateform, version,deviceToken,version1 ];
...
+ (void) parseMemberInfo:(NSDictionary*)dic
{
    ...
    if ( value && [value isKindOfClass:[NSDictionary class]] ) {
        ...
        value = [proxyDic objectForKey:@"code"];
        if ( value && value != [NSNull null] ) {
            user.idcCode = value;
        }
...
+(BOOL) getStatsData
{
    ...
    if ( [lock tryLock] ) {
        ...
        time_t t = (time_t) (lastDayLong / 1000);
...
+ (BOOL) procecssAccessData:(NSObject*)obj time:(long long)t
{
    if ( !obj || ![obj isKindOfClass:[NSDictionary class]] ) return NO;
    NSDictionary* dic = (NSDictionary*) obj;
    ...
    long long serverTime = [[result objectForKey:@"currTime"] longLongValue];
    long long lastTime = [[result objectForKey:@"lastTime"] longLongValue];
    ...
    if ( accessLogs && [accessLogs count] > 0 ) {
        ...
        [StatsDayService explainAccessLog:lt serverTime:st data:accessLogs];
    ...
    if ( value && value != [NSNull null] ) {
        ...
        if ( proxyFlag == INSTALL_FLAG_VPN_RIGHT_IDC_PAC || proxyFlag == INSTALL_FLAG_VPN_RIGHT_IDC_NO_PAC ||
            ...
            if ( [@"vpn" isEqualToString:user.stype] && ![UIDevice isVPNEnabled] ) {
...
+ (NSDictionary*) parseAccessData:(NSDictionary*)dic
{
    ...
    if ( value && value != [NSNull null] ) {
        [result setObject:value forKey:@"proxyFlag"];
    }
...
//根据用户的用户卡 来设置要发送的 查询流量的信息 参数1：用户名 ， 参数2：位置 ， 参数3：运行商 ， 参数4：套餐
- (void) getCarrierInfo:(NSString*)tel area:(NSString*)area code:(NSString*)code type:(NSString*)type
{
    ...
    //架构URL 后面加上deviceId，chl，rd，code，ver，source
    url = [TwitterClient composeURLVerifyCode:url];
    [self get:url];
}
...
//设置sim 卡 地区 ， 运行商 套餐 发送短信的号码 发送短信的内容
